...
f = 3e8 * exp(-(log(V)-log(14.5e-9^3)).^2/0.015 ...
              -(Tc-(580+273)).^2/10) + ...
       8e5 * exp(-(log(V)-log(40e-9^3)).^2/1 ...
               -(Tc-(250+273)).^2/1e3);
...
ta = 1; 
tb = 100; 
[M1, forc1, Hu1, Hc1] = FORC(f, V, Tc, 20+273, ta, ta, tau0, shape);
[M2, forc2, Hu2, Hc2] = FORC(f, V, Tc, 20+273, tb, ta, tau0, shape);
...
[a, b] = meshgrid(linspace(0, 1, length(rho2(:,1))), ...
                  linspace(0, 1, length(rho2(1,:)))); 
x = b - a; 
y = b * sqrt(log(tb/tau0)) - a * sqrt(log(ta/tau0)); 

phiq = atan(x ./ y); 
phiq(1, 1) = 0;
rq = sqrt(x.^2 + y.^2); 

[r, phi] = meshgrid(linspace(0, 2*pi), ...
                    linspace(-pi/2, pi/2)); 

F = scatteredInterpolant(rq(:), phiq(:), rho2(:)); 
rho2i = F(r, phi); 

figure(2222);
[C, h] = contourf(r, phi, abs(rho2i)); 

figure(2223);
[C, h] = contourf(r, phi, angle(rho2i));
h.LineStyle = 'none';
colormap(cyclicmap);
...
figure(5)
clf
contourf(abs(rho1),50)
figure(6) 
contourf(abs(rho2),50)

rr = rho2./rho1; 
figure(7)
contourf(abs(rr), 50); 
figure(8)
contourf(angle(rr), 50); 
